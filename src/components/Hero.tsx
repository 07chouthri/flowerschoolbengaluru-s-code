...
import { X, Mail, Check, Phone, User } from "lucide-react";
...
function FrontCard({ isVisible, onClose, onSubscribe }: {
  isVisible: boolean;
  onClose: () => void;
  onSubscribe: (data: { email: string; name: string; phone: string }) => void;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/landing/contact', { email, name, phone });

      onSubscribe({ email, name, phone });
      setIsSubmitted(true);
...
const Hero = ({ onNavigate }: HeroProps) => {
  ...
  const handleSubscribe = (data: { email: string; name: string; phone: string }) => {
